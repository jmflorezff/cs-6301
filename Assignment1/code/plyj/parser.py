...
class MyLexer(object):
    ...
    def __init__(self, comment_texts, identifiers):
    ...
    def t_LINE_COMMENT(self, t):
        r'//.*'
        self.comment_texts.append(t.value[2:])

    def t_BLOCK_COMMENT(self, t):
        r'/\*(.|\n)*?\*/'
        t.lexer.lineno += t.value.count('\n')
        self.comment_texts.append(t.value[2:][:-2])
    ...
    def t_NAME(self, t):
        ...
        if t.value in MyLexer.keywords:
            if t.value == 'package' or  t.value == 'import':
                self.ignoring = True
            t.type = t.value.upper()
        else:
            if not self.ignoring:
                self.identifiers.append(t.value)
        return t
...
class Parser(object):
    ...
    def __init__(self):
        self.comment_texts = []
        self.identifiers = []
        self.lexer = lex.lex(module=MyLexer(self.comment_texts,
                                            self.identifiers),
                             optimize=1)
